...
def main():
    ...
    # Write output
    print(f"\nWriting {len(flags)} flags to {output_path}...")
    with open(output_path, 'w') as f:
        f.write(f'"""Pre-rendered flag pixel data at {FLAG_W}x{FLAG_H}."""\n\n')
        f.write(f"FLAG_W = {FLAG_W}\n")
        f.write(f"FLAG_H = {FLAG_H}\n\n")
        f.write("# (name, continent, pixel_data) - pixel_data is tuple of rows,\n")
        f.write("# each row is tuple of (r, g, b) tuples\n")
        f.write("FLAGS = [\n")

        for continent in ['AFRICA', 'AMERICAS', 'ASIA', 'EUROPE', 'OCEANIA']:
            f.write(f"    # ===== {continent} =====\n")
            continent_flags = sorted(
                [(n, c, p) for n, (c, p) in flags.items() if c == continent]
            )
            for name, cont, pixels in continent_flags:
                f.write(f"    ({name!r}, {cont!r},\n")
                f.write(f"     (\n")
                for row in pixels:
                    f.write(f"      {row},\n")
                f.write(f"     )),\n")
    ...
    print(f"Done! {file_size / 1024:.0f} KB written ({len(flags)} flags)")
